...
namespace Microsoft.AspNetCore.Builder
{
    ...
    public static class AriesCloudApiApplicationBuilderExtensions
    {
        ...
        /// <returns></returns>
        public static IApplicationBuilder UseAriesCloudAPI(this IApplicationBuilder app, AriesCloudApiMiddlewareOptions options = null)
        {
            app.Validate();
            ...
            //if (options == null) options = new AriesCloudAPIMiddlewareOptions();
            //options.AuthenticationMiddleware(app);

            //app.UseMiddleware<MutualTlsEndpointMiddleware>();
            //app.UseMiddleware<AriesCloudAPIMiddleware>();
        ...
        internal static void Validate(this IApplicationBuilder app)
        {
            ...
            using (var scope = scopeFactory.CreateScope())
            {
                ...
                TestService(serviceProvider, typeof(IStateStore), logger, "No storage mechanism for state specified. Use the 'AddInMemoryStateStore' extension method to register a development version.");
                ...
                var persistedTenants = serviceProvider.GetService(typeof(InMemoryStateStore));
                if (persistedTenants?.GetType().FullName == typeof(InMemoryStateStore).FullName)
                {
                    logger.LogWarning("You are using the in-memory version of the persisted Tenant store. This will store consent decisions, authorization codes, refresh and reference tokens in memory only. If you are using any of those features in production, you want to switch to a different store implementation.");
                }
                else
                { 
                    //logger.LogDebug("Applying database migrations...");

                    //var context = scope.ServiceProvider.GetService<IPersistedGrantDbContext>();

                    //// apply db migrations
                    ////context.Database.Migrate(); 
                    //context.ApplyDbMigrations();
                }

                //var options = serviceProvider.GetRequiredService<AriesCloudAPIOptions>();
        ...
        //{
        //    var options = services.GetRequiredService<AriesCloudAPIOptions>();
        ...
        //    {
        //        logger.LogWarning("No authentication scheme has been set. Setting either a default authentication scheme or a CookieAuthenticationScheme on AriesCloudAPIOptions is required.");
        //    }
        //    else
        //    {
        ...
        //            logger.LogInformation("Using explicitly configured authentication scheme {scheme} for AriesCloudAPI", options.Authentication.CookieAuthenticationScheme);
        //        }
        //        else
        //        {
        //            authenticationScheme = await schemes.GetDefaultAuthenticateSchemeAsync();
        //            logger.LogInformation("Using the default authentication scheme {scheme} for AriesCloudAPI", authenticationScheme.Name);
        //        }

        //        if (!typeof(IAuthenticationSignInHandler).IsAssignableFrom(authenticationScheme.HandlerType))
        //        {
        //            logger.LogInformation("Authentication scheme {scheme} is configured for AriesCloudAPI, but it is not a scheme that supports signin (like cookies). If you support interactive logins via the browser, then a cookie-based scheme should be used.", authenticationScheme.Name);
        //        }
        ...
        //private static void ValidateOptions(AriesCloudAPIOptions options, ILogger logger)
        //{
