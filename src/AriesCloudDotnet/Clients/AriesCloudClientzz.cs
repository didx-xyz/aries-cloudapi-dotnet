...
namespace AriesCloudDotnet.Clients
{
    ...
    public class AriesCloudClient : IACAPYClient
    {
        ...
        public AriesCloudClient(IConfiguration config, ILogger<AriesCloudClient> logger)
        {
        ...
        public async Task<WalletPublicDid> WalletDidPublic()
        {
            var request = new HttpRequestMessage
            {
                ...
                RequestUri = new Uri($"{_adminUrl}{AriesCloudConstants.WalletDidPublicUri}")
            };

            if (!string.IsNullOrEmpty(_adminUrlApiKey))
            {
                request.Headers.Add(AriesCloudConstants.ApiKeyHeader, _adminUrlApiKey);
            }
        ...
        public async Task<CreatePresentationResponse> CreatePresentationRequestAsync(PresentationRequestConfiguration configuration)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(_adminUrlApiKey))
                {
                    httpContent.Headers.Add(AriesCloudConstants.ApiKeyHeader, _adminUrlApiKey);
                }

                var response = await _httpClient.PostAsync($"{_adminUrl}{AriesCloudConstants.PresentProofCreateRequest}", httpContent);
                var responseContent = await response.Content.ReadAsStringAsync();
