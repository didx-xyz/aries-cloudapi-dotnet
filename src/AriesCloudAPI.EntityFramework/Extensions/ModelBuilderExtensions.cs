...
using AriesCloudAPI.EntityFramework.Options;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Metadata.Builders;

namespace AriesCloudAPI.EntityFramework.Extensions
{
    /// <summary>
    ...
    public static class ModelBuilderExtensions
    {
        ...
        public static void ConfigurePersistedGrantContext(this ModelBuilder modelBuilder, OperationalStoreOptions storeOptions)
        {
            ...
            modelBuilder.Entity<PersistedState>(Tenant =>
            {
                Tenant.ToTable(storeOptions.PersistedTenants);

                Tenant.Property(x => x.Key).HasMaxLength(100).ValueGeneratedNever();
                Tenant.Property(x => x.Data).IsRequired();

                Tenant.HasKey(x => x.Key);

                //Tenant.HasIndex(x => new { x.SubjectId, x.ClientId, x.Type });
                //Tenant.HasIndex(x => new { x.SubjectId, x.SessionId, x.Type });
                //Tenant.HasIndex(x => x.Expiration);
            });

            //modelBuilder.Entity<PersistedTenant>(Tenant =>
            //{
            //    Tenant.ToTable(storeOptions.PersistedTenants);

            //    Tenant.Property(x => x.Key).HasMaxLength(100).ValueGeneratedNever();
            //    Tenant.Property(x => x.TenantId).HasMaxLength(100).IsRequired();
            //    Tenant.Property(x => x.TenantName).HasMaxLength(100).IsRequired();
            //    Tenant.Property(x => x.AccessToken).HasMaxLength(200).IsRequired();
            //    Tenant.Property(x => x.ImageUrl).HasMaxLength(255); 
            //    Tenant.Property(x => x.Created).IsRequired();
